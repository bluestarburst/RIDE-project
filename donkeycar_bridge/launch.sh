...
# Launch script for Donkeycar integration with CPM Lab
# This script starts all necessary components to run a Donkeycar with CPM Lab
...
if ! pgrep -f "lab_control_center" > /dev/null; then
    echo "Starting Lab Control Center..."
    cd "${PROJECT_DIR}/cpm_lab/lab_control_center"
    ./run.bash &
    # Give it time to start
    sleep 5
else
    echo "Lab Control Center is already running."
fi
...
echo "Donkeycar is now running with CPM Lab. Press Ctrl+C to stop."
wait $CONTROLLER_PID
cleanup